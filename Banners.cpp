...
{
char buff;
OpenDialog1->FileName=ExtractFilePath(Application->ExeName)+"input.html";
Edit1->Text=OpenDialog1->FileName;
OpenDialog2->FileName=ExtractFilePath(Application->ExeName)+"output.html";
Edit2->Text=OpenDialog2->FileName;
OpenDialog3->FileName=ExtractFilePath(Application->ExeName)+"filters.txt";
Edit3->Text=OpenDialog3->FileName;
StatusListBox->Items->Add("Вкажіть шляхи до файлів та натисніть \"Почати\"");
}
//---------------------------------------------------------------------------
...
input.open(inputfilename.c_str(),ios::nocreate); 		   // Відкриваємо вхідний файл
inputOut.open(inputfilename.c_str(),ios::nocreate);

if(!input.is_open()){
	StatusListBox->Items->Add("Файл "+inputfilename+" відкрити не вдалося");
		health=0;
}
else{
...
if(!output.is_open()){
	StatusListBox->Items->Add("Файл "+outputfilename+" відкрити не вдалося");
	health=0;
}
else{
...
filters.open(filtersfilename.c_str(),ios::nocreate);            				// Відкриваємо файл з фільтрами

if(!filters.is_open()){
	StatusListBox->Items->Add("Файл "+filtersfilename+" відкрити не вдалося");
	health=0;
}
else{
...
//---------------------------------------------------------------------------
void copy_banners(ifstream& input,ifstream& filters,ofstream &output){    // Функція, яка копіює банери (без вирізання)
